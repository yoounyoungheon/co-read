...
export const FeedCard = ({ image, projectName, id, href, keyword }: FeedCardProps) => {
  const targetHref = href ?? `/project?id=${id}`;

  return (
    <FlipCard
      className="min-w-[250px] transition-transform duration-300 hover:-translate-y-1 hover:scale-[1.02]"
      ...
      backCard={
        <FeedBackCard id={id} projectName={projectName} keyword={keyword} href={targetHref} />
      }
    />
  );
};
