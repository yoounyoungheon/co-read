...
이 문서는 현재 레포의 상위 아키텍처를 `web`, `signaling`, `mcp` 중심으로 정리해요.
...
`web/AGENTS.md`가 프론트엔드 작업의 전역 정책을 가져요.
...
즉 `web`은 단순 앱 디렉터리만이 아니라, 에이전트 작업 규약까지 포함한 프론트엔드 작업 공간이에요.
...
`web/AGENTS.md`에 모든 구현 상세를 다 넣으면 문서가 길어지고 실제 코드와 드리프트가 쉽게 생겨요.
...
즉 `mcp`는 코드 생성 서버가 아니라 “가이드 선택 서버”예요.
...
즉 브라우저 간 직접 media를 보내기 위한 “메시지 조정 서버”예요.
...
즉 `mcp`는 운영 트래픽 경로에 없고, 개발/자동화 경로에만 있어요.
...
즉 서비스 런타임과 개발 자동화 런타임을 분리해 유지보수성과 명확성을 높이는 구조예요.
