...
const pathMatches = (candidatePath, patterns) => {
...
const textIncludesKeyword = (text, keywords) => {
    const normalizedText = text.toLowerCase();
    return keywords.some((keyword) => normalizedText.includes(keyword.toLowerCase()));
};
const toAbsoluteSourcePath = (sourcePath) => path.resolve(repoRoot, sourcePath);
async function readGuideMarkdown(filePath) {
...
const guides = [
    {
        ...
        title: "재사용 컴포넌트 가이드",
        summary: "shared/ui 변경은 shared/ui 안에 유지하고, 재사용 가능한 공개 props를 보존하며, story는 컴포넌트 옆에 함께 둬야 합니다.",
        triggers: {
            paths: [
...
`,
    },
    {
        ...
        title: "도메인 컴포넌트 가이드",
        summary: "Feature UI는 feature/*/ui에 두고, 가능하면 shared UI를 조합해서 만들며, 같은 경로의 story를 반드시 함께 유지해야 합니다.",
        triggers: {
            paths: [
...
`,
    },
    {
        ...
        summary: "기본값은 Server Component이며, page/layout은 서버에 유지하고 데이터 조회와 초기 렌더링 책임도 서버에 둬야 합니다.",
        triggers: {
            paths: [
...
`,
    },
    {
        ...
        summary: "Client Component는 예외적으로만 사용하고, `use client` 범위를 최소화하며 상호작용이 필요한 최소 단위만 분리해야 합니다.",
        triggers: {
            paths: [],
            keywords: [
                "client component",
...
`,
    },
    {
        ...
        summary: "단순한 선택/열림/닫힘 상태는 React state 대신 radio, checkbox, label, peer-checked 조합으로 구현해 Server Component를 유지할 수 있습니다.",
        triggers: {
            paths: [
                "web/src/app/**/*.tsx",
                "web/src/app/**/*.css",
...
`,
    },
    {
        ...
        summary: "shared SSE provider는 하나의 EventSource를 관리하고, 채팅 스트리밍은 START와 PAIRING payload를 누적하다가 FINISH에서 연결을 닫습니다.",
        triggers: {
            paths: [
...
`,
    },
    {
        ...
        summary: "story는 컴포넌트 옆에 두고, Default를 먼저 제공하며, 실제 레이아웃 맥락을 재현하는 최소 wrapper만 사용해야 합니다.",
        triggers: {
            paths: [
                "web/src/app/shared/ui/**/*.stories.tsx",
                "web/src/app/feature/*/ui/**/*.stories.tsx",
...
`,
    },
    {
        ...
        title: "스타일 구현 가이드",
        summary: "스타일은 mobile-first와 가독성 우선 원칙으로 설계하고, width/flow 관점에서 원인을 먼저 파악한 뒤 Tailwind를 적용해야 합니다.",
        triggers: {
            paths: [
                "web/src/app/**/*.css",
                "web/src/app/**/*.tsx",
...
export const resolveGuides = (task, targetPaths) => {
    const normalizedTask = task.trim();
    const repoRelativeTargetPaths = targetPaths.map((targetPath) => toRepoRelativePath(targetPath));
    const matches = guides
        .map((guide) => {
        const matchedPaths = repoRelativeTargetPaths.filter((targetPath) => pathMatches(targetPath, guide.triggers.paths));
        const keywordMatched = textIncludesKeyword(normalizedTask, guide.triggers.keywords);
        const storySignal = guide.id === "storybook-authoring" &&
...
export const readGuide = async (id) => {
    const guide = guideMap.get(id);
    if (!guide) {
        throw new Error(`Unknown guide id: ${id}`);
    }
    const sources = await Promise.all(guide.sources.map(async (sourcePath) => {
    ...
    const guideMarkdownSections = await Promise.all((guide.guideFiles ?? []).map(async (filePath) => ({
        filePath,
    ...
    const appendedGuideMarkdown = guideMarkdownSections
        ...
        .join("\n\n---\n\n");
    const existingCanonicalSources = canonicalSources.filter((source) => source.lastModified !== null);
